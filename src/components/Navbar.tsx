  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] h-14 bg-background/95 border-b border-border flex items-center justify-between px-6 md:px-12 backdrop-blur-md">
      ...
      <div className="hidden md:flex items-center gap-8">
        {links.map((item) => (
          <a key={item.label} href={item.href} className="text-[12px] tracking-[2px] text-dim uppercase hover:text-foreground transition-colors flex items-center gap-2">
            {item.label}
            {item.soon && <span className="text-[9px] tracking-[2px] uppercase border px-1.5 py-0.5 rounded" style={{ color: "hsl(40 90% 60%)", borderColor: "hsla(40,90%,60%,0.5)" }}>Beta</span>}
          </a>
        ))}
      ...
      {menuOpen && (
        <div className="absolute top-14 left-0 right-0 bg-background border-b border-border flex flex-col items-center gap-5 py-6 md:hidden">
          {links.map((item) => (
            <a key={item.label} href={item.href} className="text-[14px] tracking-[2px] text-dim uppercase flex items-center gap-2" onClick={() => setMenuOpen(false)}>
              {item.label}
              {item.soon && <span className="text-[9px] tracking-[2px] uppercase border px-1.5 py-0.5 rounded" style={{ color: "hsl(40 90% 60%)", borderColor: "hsla(40,90%,60%,0.5)" }}>Beta</span>}
            </a>
          ))}
