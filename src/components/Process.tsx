const steps = [
  ...
  { num: "03", title: "Quote & Approve", desc: "Flat-rate pricing upfront. No surprises. You approve before anything happens. Diagnosis fee waived if you proceed." },
...
const Process = () => {
  return (
    <section id="process" className="px-6 md:px-20 py-20 md:py-24 bg-background">
      <div className="text-[10px] tracking-[6px] text-faint uppercase mb-3">// how it works</div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 relative">
        ...
        {steps.map((step) => (
          <div key={step.num} className="relative z-[1] text-center px-5 group mb-8 lg:mb-0">
            ...
            <div className="font-display text-xl tracking-[2px] text-foreground mb-2.5">{step.title}</div>
            <p className="text-[11px] text-dim leading-[1.8] tracking-[0.5px]">{step.desc}</p>
          </div>
        ))}
      </div>
    </section>
